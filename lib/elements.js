...
function containsVerb(word) {
  return !!word.match(/^(get|update|create|remove|destroy)_/)
}
...
Collection.prototype.first = function(num, cb) {
  ...
  return invoke(function(done) {
    this._fetchItems(function(items) {
      console.log('got items', items, num)
...
LinkedElement.prototype.get = function(params, cb) {
  ...
  return invoke(function(done) {
    return this._client.request(this._link, params).then(function(result) {
      ...
      var proxy = new Proxy(new Element(this.name, this._client, result), proxyHandler)
      done(proxy)
    }.bind(this))
  }.bind(this))
}
...
LinkedCollection.prototype.get = function(cb) {
  return invoke(function(done) {
    return this._fetch(function(res) {
      var element = new Element(this.name, this._client, res);
  ...
  var fn = function find() {
    return invoke(function(done) {
      ...
      this._client.request(link, { id: id }).then(function(result) {
        var proxy = buildEmbeddedProxy(this._client, result._class[0], result);
        done(proxy);
      }.bind(this))
    }.bind(this), cb)
  }.bind(this)
...
function buildLinkProxy(client, name, link, singular) {
  if (link.type || (link.method && (link.method.toLowerCase() != 'get' || containsVerb(name))))
...
function handleInspection(target, name) {
  console.log('checking for ' + target.constructor.name + ': ' + name.toString(), '--> ' + typeof target[name]);

  if (name == 'constructor')
    return target.constructor;
  else if (name.toString() == 'valueOf')
    return 'valueof'; // [target.constructor.name, target.name].join(':')
    ...
    return 'xx'; // target.constructor.name;
...
let proxyHandler = {
  get: function(target, name) {
    ...
    if (typeof name == 'symbol')
      return target;
    // else if (typeof target[name] == 'function' && name != 'constructor')
